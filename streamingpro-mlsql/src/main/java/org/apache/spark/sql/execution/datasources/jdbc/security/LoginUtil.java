...
public class LoginUtil {
    ...
    private static final Logger LOG = Logger.getLogger(org.apache.spark.sql.execution.datasources.jdbc.security.LoginUtil.class);
    ...
    public static synchronized UserGroupInformation login(String userPrincipal, String userKeytabPath,
                                          String krb5ConfPath, Configuration conf) throws IOException {
        LOG.info(String.format("Login start!!  userPrincipal:%s ,userKeytabPath:%s, krb5ConfPath:%s", userPrincipal, userKeytabPath, krb5ConfPath));
        ...
        LOG.info("Login setConfiguration start !!!!!!!!!!!!!!");
        setKrb5Config(krb5ConfFile.getAbsolutePath());
        setConfiguration(conf);
        LOG.info("Login setConfiguration success !!!!!!!!!!!!!!");
        // 4.login and check for hadoop
        UserGroupInformation ugi =null;
        if (checkNeedLogin(userPrincipal)) {
            ugi = loginHadoop(userPrincipal, userKeytabFile.getAbsolutePath());
            isCheck = true;
        }
    ...
    private static boolean checkNeedLogin(String principal) throws IOException {
        ...
        try {
            ...
            LOG.error("获取当前用户失败提醒，自动做重新登录操作");
            return true;
        }

        if ((currentUser != null) && (currentUser.hasKerberosCredentials())) {
            if (checkCurrentUserCorrect(principal) && !currentUser.isFromKeytab()) {
                LOG.error("current user is not from keytab.");
                throw new IOException("current user is not from keytab.");
            } else if (checkCurrentUserCorrect(principal) && currentUser.isFromKeytab()) {
                return false;
            } else {
    ...
    private static void setKrb5Config(String krb5ConfFile) throws IOException {
        LOG.info("current krb5ConfFile:" + krb5ConfFile);
    ...
    public static void setJaasConf(String loginContextName, String principal, String keytabFile) throws IOException {
        ...
        for (int i = 0; i < entrys.length; i++) {
            ...
            if (IS_IBM_JDK) {
                ...
                File a = new File(entrys[i].getOptions().get("keyTab").toString());
                File b = new File(keytabFile);
