...
import com.code.common.resource.loader.hdfs.{HDFSClient, HDFSResourceLoader}
import com.code.common.resource.{ClassLoaderRegistry, JdbcDriverRegistry, ResourceLoader}
 ...
 * @author Pan Jiebin
 */
object JdbcHelper extends Logging {

  var driverRegistry: JdbcDriverRegistry = JdbcDriverRegistry.getInstance()

  val JDBC_DRIVER_TYPE: String = "db_type"
  val JDBC_DRIVER_VERSION: String = "db_version"
  val SUPPORT_JDBC_MULTI_VERSION: String = "support_mv"
  val HDFS_ADDRESS: String = "spark.my.hdfs.address"
  val HDFS_LOGIN_USER: String = "spark.my.hdfs.user"
  val HDFS_BASE_DIR: String = "spark.my.hdfs.baseDir"
  ...
  def init() {
    if (SparkEnv.get.conf.contains(HDFS_ADDRESS)) {
      val address = SparkEnv.get.conf.get(HDFS_ADDRESS)
      val user = SparkEnv.get.conf.get(HDFS_LOGIN_USER)
      val dir = SparkEnv.get.conf.get(HDFS_BASE_DIR)
      val registry: ClassLoaderRegistry = ClassLoaderRegistry.getInstance()
      val client: HDFSClient = new HDFSClient(address, user)
      val resourceLoader: ResourceLoader = new HDFSResourceLoader(client)
      registry.setDefaultLoader(resourceLoader)
      registry.register(dir)
    } else {
      if (log.isWarnEnabled()) {
        log.warn("[{}] un set, could not support jdbc input multi version", HDFS_ADDRESS)
      }
    }
  }
  ...
  def createConnectionFactory(options: JDBCOptions): () => Connection = {
    ...
    () => {
      ...
      val support = properties.getProperty(SUPPORT_JDBC_MULTI_VERSION)
      if (support != null && "true".equals(support)) {
        //support jdbc driver multi version load
        val driverType = properties.getProperty(JDBC_DRIVER_TYPE)
        val driverVersion = properties.getProperty(JDBC_DRIVER_VERSION)
        driver = driverRegistry.get(driverType, driverVersion, driverClass)
        require(driver != null,
          s"Could not find jdbc driver[type = $driverType, version = $driverVersion].")
      } else {
        //spark origin load driver mode
        DriverRegistry.register(driverClass)
        driver = DriverManager.getDrivers.asScala.collectFirst {
